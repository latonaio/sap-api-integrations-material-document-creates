...
import (
	sap_api_caller "sap-api-integrations-material-document-creates/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-material-document-creates/SAP_API_Input_Reader"
	"sap-api-integrations-material-document-creates/config"
...
func main() {
	...
	inputSDC := fr.ReadSDC("./Inputs/SDC_Material_Document_Header_sample.json")
	accepter := getAccepter(inputSDC)
	header := inputSDC.ConvertToHeader()
	item := inputSDC.ConvertToItem()

	caller.AsyncPostMaterialDocument(
		header,
		item,
		accepter,
	)
