...
import (
	...
	sap_api_output_formatter "sap-api-integrations-material-document-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callMaterialDocumentSrvAPIRequirementHeader("A_MaterialDocumentHeader", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaterialDocumentSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_MaterialDocument('%s')/to_Item", item.MaterialDocument)
	outputDataItem, err := c.callMaterialDocumentSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaterialDocumentSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
